...
function CarOption() {
	...
	if (!token) {
		return (
			<Row className="mt-4">
				<Col>
					<h1 className="text-center">Please login first to get car options data!</h1>
				</Col>
			</Row>
		);
	}
	...
	return (
		<>
			<div className="d-flex justify-content-between align-items-center mt-4">
				...
				{user && user.role_id === 1 && (
					<Button variant="primary" onClick={() => navigate({ to: '/admin/caroptions/create' })} className="d-flex align-items-center">
				...
				<Table striped bordered hover className="mt-4">
					...
					<tbody>
						{carOptions.map(carOption => (
							<CarOptionItem carOption={carOption} key={carOption?.id} refetchData={refetchData} />
						))}
					</tbody>
				</Table>
			)}
